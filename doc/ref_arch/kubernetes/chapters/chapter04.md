...
|`ra2.k8s.012`|Kubernetes API-s|Kubernetes [Alpha API](https://kubernetes.io/docs/reference/using-api/#api-versioning) are recommended only for testing, therefore all Alpha API-s **must** be disabled.|[req.int.api.03](./chapter02.md#22-reference-model-requirements)||
|`ra2.k8s.013`|Kubernetes API-s|To provide backward compatibility all supported GA and Beta API-s of Kubernetes **must** be enabled. |[req.int.api.04](./chapter02.md#22-reference-model-requirements)||
